...
import { CheckCircle, Search, Plus, Eye, Edit, Calendar, User } from 'lucide-react';
...
export default function QualityPage() {
  const [qualityPolicies, setQualityPolicies] = useState<Policy[]>([]);
  ...
  useEffect(() => {
    // Filter quality policies based on search term
    const filtered = qualityPolicies.filter(policy =>
      policy.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      policy.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      policy.tags.some(tag => tag.name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    setFilteredPolicies(filtered);
  }, [qualityPolicies, searchTerm]);

  const fetchQualityPolicies = async () => {
    try {
      setLoading(true);
      console.log('Fetching Quality policies with category=Quality');
      const response = await fetch('/api/policies?category=Quality');
      const result = await response.json();
      console.log('API response:', result);

      if (response.ok) {
        setQualityPolicies(result.data || []);
        console.log('Quality policies loaded:', result.data?.length || 0);
      } else {
        setError(result.message || 'Failed to fetch quality policies');
      }
    } catch (error) {
      console.error('Error fetching quality policies:', error);
      setError('An error occurred while fetching quality policies');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-4">
        ...
        <div className="rounded p-4 mb-4 text-white position-relative" style={{ backgroundColor: "#ca1f27" }}>
          <div className="row align-items-center">
            <div className="col-md-8">
              <h2 className="mb-3">Quality</h2>
              <p className="mb-4 opacity-90">
                Access quality assurance policies, standards, procedures, and quality control guidelines.
              </p>
            </div>
            <div className="col-md-4 text-end">
              <CheckCircle size={120} className="opacity-25" />
            </div>
          </div>
        </div>
        ...
        <div className="p-3 border-bottom">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h5 className="mb-1">Quality Management</h5>
              <small className="text-muted">
                {filteredPolicies.length} quality document{filteredPolicies.length !== 1 ? 's' : ''} available
              </small>
            </div>
          ...
          {filteredPolicies.length === 0 ? (
            <div className="text-center py-5">
              ...
              <h5 className="text-muted">
                {searchTerm ? 'No quality policies found matching your search' : 'No quality policies found'}
              </h5>
              {!searchTerm && (
                <div>
                  <p className="text-muted mb-3">
                    No quality policies have been created yet. Create your first quality policy to get started.
                  </p>
                  <button 
                  ...
                  >
                    <Plus size={16} className="me-2" />
                    Create Your First Quality Policy
                  </button>
                </div>
              )}
            </div>
                    ...
                    <div className="card-body">
                      ...
                      <div className="mb-3">
                        <div className="mb-2">
                          ...
                          <span className="badge bg-info text-white ms-2">Quality</span>
                        </div>
