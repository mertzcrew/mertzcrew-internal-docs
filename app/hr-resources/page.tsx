...
import { Users, Search, Plus, Eye, Edit, Calendar, User } from 'lucide-react';
...
export default function HRResourcesPage() {
  ...
  useEffect(() => {
    // Filter HR policies based on search term
    const filtered = hrPolicies.filter(policy =>
      policy.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      policy.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      policy.tags.some(tag => tag.name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    setFilteredPolicies(filtered);
  }, [hrPolicies, searchTerm]);

  const fetchHRPolicies = async () => {
    try {
      setLoading(true);
      console.log('Fetching HR policies with category=HR');
      const response = await fetch('/api/policies?category=HR');
      const result = await response.json();
      console.log('API response:', result);

      if (response.ok) {
        setHrPolicies(result.data || []);
        console.log('HR policies loaded:', result.data?.length || 0);
      } else {
        setError(result.message || 'Failed to fetch HR policies');
      }
    } catch (error) {
      console.error('Error fetching HR policies:', error);
      setError('An error occurred while fetching HR policies');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-4">
        ...
        <div className="rounded p-4 mb-4 text-white position-relative" style={{ backgroundColor: "#ca1f27" }}>
          <div className="row align-items-center">
            <div className="col-md-8">
              <h2 className="mb-3">HR Resources</h2>
              <p className="mb-4 opacity-90">
                Access HR policies, benefits information, employment guidelines, and human resources documentation.
              </p>
            </div>
            <div className="col-md-4 text-end">
              <Users size={120} className="opacity-25" />
            </div>
          </div>
        </div>
        ...
        <div className="p-3 border-bottom">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h5 className="mb-1">HR Policies & Documentation</h5>
              <small className="text-muted">
                {filteredPolicies.length} HR polic{filteredPolicies.length !== 1 ? 'ies' : 'y'} available
              </small>
            </div>
          ...
          {filteredPolicies.length === 0 ? (
            <div className="text-center py-5">
              ...
              <h5 className="text-muted">
                {searchTerm ? 'No HR policies found matching your search' : 'No HR policies found'}
              </h5>
              {!searchTerm && (
                <div>
                  <p className="text-muted mb-3">
                    No HR policies have been created yet. Create your first HR policy to get started.
                  </p>
                  <button 
                    ...
                    <div className="card-body">
                      ...
                      <div className="mb-3">
                        <div className="mb-2">
                          ...
                          <span className="badge bg-success ms-2">HR</span>
                        </div>
