...
import { BookOpen, Search, Plus, Eye, Edit, Calendar, User } from 'lucide-react';
...
export default function TrainingPage() {
  const [documentationPolicies, setDocumentationPolicies] = useState<Policy[]>([]);
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated') {
      fetchDocumentationPolicies();
    }
  }, [status, session, router]);

  useEffect(() => {
    // Filter documentation policies based on search term
    const filtered = documentationPolicies.filter(policy =>
      policy.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      policy.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      policy.tags.some(tag => tag.name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    setFilteredPolicies(filtered);
  }, [documentationPolicies, searchTerm]);

  const fetchDocumentationPolicies = async () => {
    try {
      setLoading(true);
      console.log('Fetching Documentation policies with category=Documentation');
      const response = await fetch('/api/policies?category=Documentation');
      const result = await response.json();
      console.log('API response:', result);

      if (response.ok) {
        setDocumentationPolicies(result.data || []);
        console.log('Documentation policies loaded:', result.data?.length || 0);
      } else {
        setError(result.message || 'Failed to fetch documentation policies');
      }
    } catch (error) {
      console.error('Error fetching documentation policies:', error);
      setError('An error occurred while fetching documentation policies');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-4">
          ...
          <div className="row align-items-center">
            <div className="col-md-8">
              ...
              <p className="mb-4 opacity-90">
                Access training materials, guides, manuals, and educational documentation.
              </p>
            </div>
        ...
        <div className="p-3 border-bottom">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h5 className="mb-1">Documentation & Training Materials</h5>
              <small className="text-muted">
                {filteredPolicies.length} document{filteredPolicies.length !== 1 ? 's' : ''} available
              </small>
            </div>
          ...
          {filteredPolicies.length === 0 ? (
            <div className="text-center py-5">
              ...
              <h5 className="text-muted">
                {searchTerm ? 'No documentation found matching your search' : 'No documentation found'}
              </h5>
              {!searchTerm && (
                <div>
                  <p className="text-muted mb-3">
                    No documentation has been created yet. Create your first documentation to get started.
                  </p>
                  <button 
                    ...
                    <div className="card-body">
                      ...
                      <div className="mb-3">
                        <div className="mb-2">
                          ...
                          <span className="badge bg-info ms-2">Documentation</span>
                        </div>
