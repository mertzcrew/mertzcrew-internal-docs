...
export const authOptions = {
  ...
  callbacks: {
          ...
          if (email.endsWith('@mertzcrew.com')) {
            if (!user) {
              ...
              user = new User({
                ...
                permissions: ['view_policy', 'edit_policy'],
                isEmailVerified: true, // Google OAuth means email is verified
                isActive: true
              });
              await user.save();
              console.log('Created new user for Google OAuth:', email);
            }
            try { await User.updateOne({ _id: user._id }, { $set: { lastLogin: new Date() } }); } catch(e) { console.warn('lastLogin update error', e?.message); }
            return true;
    ...
    async session({ session, token }) {
      if (token) {
        // Prioritize our database ID over OAuth provider ID
